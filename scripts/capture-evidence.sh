...
#   capture-evidence.sh --workspace <path> --run-id <id> --no-upload
#
# Default mode builds the bundle and uploads it directly (manual/local
# capture). --no-upload builds the bundle only and prints its local
# path, so a caller (the pipeline) can sign it before uploading the
# signed set together.
...
[[ -z "$WORKSPACE" || -z "$RUN_ID" ]] && {
  echo "Usage: $0 --workspace <path> --run-id <id> [--vault <bucket> [--profile <p>] | --no-upload]" >&2
  exit 2
}
[[ -z "$NO_UPLOAD" && -z "$VAULT" ]] && {
  echo "Error: --vault is required unless --no-upload is set" >&2
  exit 2
}
